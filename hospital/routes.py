...
def generate_pdf():
    ...
    rendered_html = render_template('print_presc.html',patient  = registred_patient[username])
    path_to_wkhtmltopdf = r'C:\Users\w11\hospital\env\wkhtmltopdf\bin\wkhtmltopdf.exe'

    pdf = pdfkit.from_string(rendered_html, False, configuration=pdfkit.configuration(wkhtmltopdf=path_to_wkhtmltopdf))
...
def presc():
    username = session.get('username')
    if username in registred_patient:
        return render_template('print_presc.html',patient = registred_patient[username])
    else:
        return render_template('error.html',r = 'C')
